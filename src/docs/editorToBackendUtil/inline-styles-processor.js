...
function _fullfilIntersection(block) {
    ...
    for (let i = 0; i < sortedEntityRanges.length; i++) {
        ...
        for (let j = 0; j < sortedISRanges.length; j++) {
            ...
            let nextEntityOffset = _.get(
                sortedEntityRanges,
                [i + 1, 'offset'],
                0
            )
            let nextEntityLength = _.get(
                sortedEntityRanges,
                [i + 1, 'length'],
                0
            )
            ...
            if (
                ...
                nextEntityOffset + nextEntityLength >
                    inlineOffset + inlineLength && // <a><strong></a></strong>
                ...
                splitedISInline.push({
                    ...
                    replace: [
                        ...
                        },
                        {
                            length:
                                nextEntityOffset -
                                (entityOffset + entityLength),
                            offset: entityOffset + entityLength,
                            style: inlineStyle,
                        },
                        {
                            length:
                                inlineOffset + inlineLength - nextEntityOffset,
...
function _entityTag(entityTagMap, entityMap, entityRanges, tagInsertMap = {}) {
    _.forEach(entityRanges, (range) => {
        ...
        // special case
        if (type === ENTITY_LIST.ANNOTATION.type) {
